...
| `with_echo_disabled_clears_and_restores_echo`               | Opens a real PTY pair, asserts `ECHO` is set on the master initially, asserts it is cleared inside the `with_echo_disabled` closure, then asserts it is restored after the call returns   |
| `with_echo_disabled_runs_body_when_tcgetattr_fails`         | Passes an invalid FD (`-1`) to `with_echo_disabled` and asserts the body closure still executes — guards the soft-fail contract that a termios error must not suppress the polyglot write |
