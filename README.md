...
- **Navbar (left sidebar)** — a "Cards" section with a `+` menu button. Clicking the button opens a dropdown with two options: **Terminal** (spawns a PTY session) and **Dungeon** (connects to the Python sidecar). Each card is a `Tabs.Tab` whose label is rendered by the `SessionCard` component. The active card receives a dark background and a bold title; inactive cards are transparent with a medium-weight title. The label shows the session slug, an optional status subtitle, `repo:branch • path-tail`, and PR / Issue badges below a divider at reduced opacity. Clicking the tab activates the corresponding card; the `×` button in the top-right of each card removes it. Removing a terminal card kills its PTY session; removing a dungeon card has no PTY side effect.
