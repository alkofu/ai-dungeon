...
- **Main pane** — one `Tabs.Panel` per card. Terminal cards contain an xterm.js terminal connected to a real shell via Tauri IPC; inactive panels are hidden with CSS (`display: none`) but remain mounted, keeping PTY sessions alive. Dungeon cards render a `DungeonPanel` with **Hi** and **Clean** buttons: pressing Hi sends a message to the Python sidecar over piped stdio and displays its reply; pressing Clean resets the display.
