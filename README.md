...
- **Live session metadata** — each terminal card holds two independent context slots: `claudeContext` (populated by OSC 6800 — Claude session metadata: slug, repo, branch, PR/issue numbers) and `shellContext` (populated by OSC 7 + OSC 7337 — shell CWD and git context). The sidebar card renders whichever slot is available, preferring `claudeContext` over `shellContext`, with mock placeholder data as the fallback. All incoming data is validated before it enters app state. See [docs/project-structure.md](docs/project-structure.md#osc-session-context-flow-6800--7--7337).
