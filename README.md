...
- **Keyboard tab navigation** — global shortcuts work whether focus is in the terminal or the sidebar. `Cmd+ArrowLeft` / `Cmd+ArrowRight` (macOS) or `Ctrl+ArrowLeft` / `Ctrl+ArrowRight` (Windows/Linux) cycle tabs with wrap-around. `Cmd/Ctrl+1` through `Cmd/Ctrl+9` jump directly to a tab by position. All eleven shortcuts suppress the WebView's default back/forward navigation unconditionally. Session cards 1–9 show a `⌘N` / `Ctrl+N` shortcut chip in the top-right corner after the modifier key has been held for ~250 ms; the chip disappears when the modifier is released. Cards at position 10 or beyond never show a chip.
...
- **Resizable sidebar** — a draggable vertical handle on the right edge of the sidebar lets users set the sidebar width between 160 px and 600 px. Keyboard users can focus the handle (Tab) and nudge it with `ArrowLeft` / `ArrowRight` in 10 px increments. The chosen width persists across restarts via `settings.json` (`layout.navbarWidth`). The handle is hidden and unfocusable when the sidebar is collapsed via the Burger toggle.
...
- **Live session context in the navbar** — each `SessionCard` in the sidebar shows git context (repo name + active branch) as a subtitle below the session slug, updated after every command via OSC 7 and OSC 7337 escape sequences. The subtitle renders `repo • branch` when both are present, just `branch` when only the branch is known, and is omitted entirely when neither is available. The full working-directory path is accessible via a tooltip on the branch text. Supported shells: bash and zsh. Fish and csh are not supported in this iteration.
...
- **Navbar (left sidebar)** — a "Cards" section with a `+` menu button. Clicking the button opens a dropdown with two options: **Terminal** (spawns a PTY session) and **Dungeon** (connects to the Python sidecar). Each card is a `Tabs.Tab` whose label is rendered by the `SessionCard` component. Cards have six visual variants: default, has-issue, has-PR, has-both, active (blue-tinted background), and needs-review (orange accent and "NEEDS REVIEW" label). All cards share a 4 px left-border accent (blue by default, orange in the needs-review variant). A `⌘N` / `Ctrl+N` shortcut chip appears in the top-right of cards at positions 1–9 while the modifier key is held (~250 ms threshold); it hides when released. PR and Issue badges occupy a reserved slot of fixed height so card height is constant whether or not badges are present. A `×` close button is hidden by default and revealed on hover or keyboard focus (top-right of the card); clicking it removes the card. Removing a terminal card kills its PTY session; removing a dungeon card has no PTY side effect.
...
| Sidebar width      | 160–600 px          | 250     | Set by dragging the sidebar resizer handle or using `ArrowLeft` / `ArrowRight` while it is focused. Persisted as `layout.navbarWidth` in `settings.json`. |
